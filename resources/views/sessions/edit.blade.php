...
<div class="container mx-auto px-4 py-8">
    <div class="max-w-md mx-auto bg-white dark:bg-gray-800 shadow-md rounded-lg overflow-hidden">
        ...
        <form method="POST" action="{{ route('sessions.update', $session) }}" class="px-6 py-4">
            @csrf
            @method('PUT')

            <div class="mb-4">
                <label for="salle" class="block text-sm font-medium text-gray-700 dark:text-gray-300">Salle</label>
                <input type="text" name="salle" id="salle" value="{{ old('salle', $session->salle) }}" class="mt-1 block w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 dark:bg-gray-700 dark:text-white" required>
                @error('salle')
            ...
            <div class="mb-4">
                ...
                <input type="date" name="date_session" id="date_session" value="{{ old('date_session', $session->date_session) }}" class="mt-1 block w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 dark:bg-gray-700 dark:text-white" required>
                @error('date_session')
                    <p class="mt-1 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div class="mb-4">
                ...
                <input type="time" name="heure_debut" id="heure_debut" value="{{ old('heure_debut', $session->heure_debut) }}" class="mt-1 block w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 dark:bg-gray-700 dark:text-white" required>
                @error('heure_debut')
                    <p class="mt-1 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div class="mb-4">
                ...
                <input type="time" name="heure_fin" id="heure_fin" value="{{ old('heure_fin', $session->heure_fin) }}" class="mt-1 block w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 dark:bg-gray-700 dark:text-white" required>
                @error('heure_fin')
                    <p class="mt-1 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div class="mb-4">
                ...
                <select name="class_id" id="class_id" class="mt-1 block w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 dark:bg-gray-700 dark:text-white" required>
                    ...
                    @foreach($classes as $class)
                        <option value="{{ $class->id }}" {{ old('class_id', $session->class_id) == $class->id ? 'selected' : '' }}>{{ $class->nom_classe }}</option>
                    @endforeach
                </select>
                @error('class_id')
                    <p class="mt-1 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>
            ...
            <div class="mb-4">
                ...
                <select name="module_id" id="module_id" class="mt-1 block w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 dark:bg-gray-700 dark:text-white" required>
                    ...
                    @foreach($modules as $module)
                        <option value="{{ $module->id }}" {{ old('module_id', $session->module_id) == $module->id ? 'selected' : '' }}>{{ $module->nom }}</option>
                    @endforeach
                </select>
                @error('module_id')
                    <p class="mt-1 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>
            ...
            <div class="mb-4">
                ...
                <select name="teacher_id" id="teacher_id" class="mt-1 block w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 dark:bg-gray-700 dark:text-white" required>
                    ...
                    @foreach($teachers as $teacher)
                        <option value="{{ $teacher->id }}" {{ old('teacher_id', $session->teacher_id) == $teacher->id ? 'selected' : '' }}>{{ $teacher->nom }} {{ $teacher->prenom }}</option>
                    @endforeach
                </select>
                @error('teacher_id')
                    <p class="mt-1 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>
